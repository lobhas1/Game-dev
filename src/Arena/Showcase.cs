...
public static class Showcase
{
    ...
    public static string CoverageDoc(IReadOnlyList<string> inputPaths)
    {
        ...
        sb.AppendLine("# M2 — vocabulary coverage (generated by `showcase-batch`; do not hand-edit)");
        sb.AppendLine();
        sb.AppendLine($"Corpus: the {inputPaths.Count} showcased spells (6 seeds + {inputPaths.Count - 6} fusions). Every verb, delivery, element, status, and band value occurring in the corpus, with the spells that exhibit it. Band families are reflected from `BalanceTables`.");
